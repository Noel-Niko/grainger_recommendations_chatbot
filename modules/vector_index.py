...
from bedrock_initializer import bedrock_runtime, check_and_initialize_llm
...
class Document:
    ...
    def get_instance(cls, **kwargs):
        ...
        if cls._vector_index is None:
            ...
            bedrock_embeddings = cls.initialize_bedrock()
            documents = []
            ...
            cls._vector_index = FAISS.from_documents(documents=documents, embedding=bedrock_embeddings)
            end_time = time.time()
            time_taken = end_time - start_time
            logging.info(f"Created FAISS vector store from structured documents in {time_taken} seconds.")

        return cls._vector_index, cls._llm
